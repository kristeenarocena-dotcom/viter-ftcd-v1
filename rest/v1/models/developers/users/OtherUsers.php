...
class OtherUsers
{
    ...
    public $otheruser_password;
    public $otheruser_key;
...
{
    try {
        $sql  = "select * from {$this->tblOtherUsers}";
        $sql .= " where otheruser_key = :otheruser_key";
        $query = $this->connection->prepare($sql);
        $query->execute(["otheruser_key" => $this->otheruser_key]);
    } catch (PDOException $e) {
        $query = false;
    }
    return $query;
...
public function setPassword()
{
    try {
        $sql  = "update {$this->tblOtherUsers} set";
        $sql .= " otheruser_key = '',";
        $sql .= " otheruser_password = :otheruser_password,";
        $sql .= " otheruser_updated  = :otheruser_updated";
        $sql .= " where otheruser_key = :otheruser_key";
        $query = $this->connection->prepare($sql);
        $query->execute([
            "otheruser_password" => $this->otheruser_password,
            "otheruser_updated"  => $this->otheruser_updated,
            "otheruser_key"      => $this->otheruser_key,
        ]);
    } catch (PDOException $e) {
        $query = false;
    }
